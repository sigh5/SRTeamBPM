	...
	if (0 >= m_pInfoCom->Get_Hp())
	{
		if (false == m_bDead)
		{
			...
			case 0:
				...
				::PlaySoundW(L"Anubis_Death_01.wav", SOUND_MONSTER, g_fSound * 2.f);
				break;
			case 1:
				::StopSound(SOUND_MONSTER);
				::PlaySoundW(L"Anubis_Death_02.wav", SOUND_MONSTER, g_fSound * 2.f);
				break;
			}
			Drop_Item(rand() % 3);
		m_bDead = true;
...
void CAnubis::Collision_Event()
{
		...
		m_pColliderCom->Check_Lay_InterSect(m_pBufferCom, m_pDynamicTransCom, g_hWnd))
	{
		...
		case 0:
			::StopSound(SOUND_MONSTER);
			::PlaySoundW(L"Anubis_pain_01.wav", SOUND_MONSTER, g_fSound + 0.2f);
			break;

		case 1:
			::StopSound(SOUND_MONSTER);
			::PlaySoundW(L"Anubis_pain_02.wav", SOUND_MONSTER, g_fSound + 0.2f);
			break;

		case 2:
			::StopSound(SOUND_MONSTER);
			::PlaySoundW(L"Anubis_pain_03.wav", SOUND_MONSTER, g_fSound + 0.2f);
			break;
		}
		}
	}
...
void CAnubis::Attack_Thunder(const _float& fTimeDelta)
{
	...
	if (5 == m_pAttackAnimationCom->m_iMotion)
	{
		if (false == m_bCreateOneThunder)
		{
			...
			::PlaySoundW(L"thunder_spell_loop.wav", SOUND_EFFECT, g_fSound + 0.2f);
	...
	if (11 == m_pAttackAnimationCom->m_iMotion)
	{
		if (false == m_bCreateTwoThunder)
		{
			...
			::StopSound(SOUND_EFFECT);
			::PlaySoundW(L"thunder_spell_cast.wav", SOUND_EFFECT, g_fSound + 0.2f);
...
void CAnubis::Attack_Stormball(const _float& fTimeDelta)
{
	...
	if (10 == m_pAttackAnimationCom->m_iMotion)
	{
		...
		if (false == m_bCreateOneStormball)
		{
			...
			case 0:
				::StopSound(SOUND_EFFECT);
				::PlaySoundW(L"lightning_strike_1.wav", SOUND_EFFECT, g_fSound * 2.f);
				break;

			case 1:
				::StopSound(SOUND_EFFECT);
				::PlaySoundW(L"lightning_strike_2.wav", SOUND_EFFECT, g_fSound * 2.f);
				break;

			case 2:
				::StopSound(SOUND_EFFECT);
				::PlaySoundW(L"lightning_strike_3.wav", SOUND_EFFECT, g_fSound * 2.f);
				break;
			}
