...
#include "Status_UI.h"
#include "Player.h"
CEquipYeti::CEquipYeti(LPDIRECT3DDEVICE9 pGraphicDev)
	:CEquipmentBase(pGraphicDev)
{
}


CEquipYeti::~CEquipYeti()
{
}

HRESULT CEquipYeti::Ready_Object(_uint iX, _uint iZ)
{
...
_int CEquipYeti::Update_Object(const _float & fTimeDelta)
{
...
void CEquipYeti::LateUpdate_Object(void)
{
	if (m_bIsWorld)
	{
...
void CEquipYeti::Render_Obejct(void)
{
	if (m_bIsWorld)
	{
...
void CEquipYeti::Collision_Event()
{
...
void CEquipYeti::Set_MouseToInventory()
{
	POINT		ptMouse{};
...
void CEquipYeti::PickingMouseUp()
{
...
void CEquipYeti::SearchInventorySlot(CInventory_UI ** pInven)
{
	POINT		ptMouse{};
	...
	if (m_EquipState != EquipState_Equip_Pet)
	{
		...
		if (PtInRect(&Rc, ptMouse))
		{
			...
			{
				Change_Equip();
				m_EquipState = EquipState_Equip_Pet;
...
_bool CEquipYeti::EquipIconPicking()
{
	CInventory_UI* pInven = static_cast<CInventory_UI*>(Get_GameObject(L"Layer_UI", L"InventoryUI"));

	const _uint iCurrentPickingID = pInven->Get_Current_Picking_ItemID();

	if (!(iCurrentPickingID == ID_YETI || iCurrentPickingID == 0))
		return false;
	...
	if (Get_DIMouseState(DIM_LB) & 0x80)
	{
		...
		{
			pInven->Set__Current_Picking_ItemID(ID_YETI);
			m_bPickingEnd = false;
			m_bisPicking = true;
			return true;
		}
...
HRESULT CEquipYeti::Add_Component(void)
{
	m_pTransCom = CAbstractFactory<CTransform>::Clone_Proto_Component(L"Proto_TransformCom", m_mapComponent, ID_DYNAMIC);
	m_pTextureCom = CAbstractFactory<CTexture>::Clone_Proto_Component(L"Proto_Equip_YetiTexture", m_mapComponent, ID_STATIC);
...
CEquipYeti * CEquipYeti::Create(LPDIRECT3DDEVICE9 pGraphicDev, _uint iX, _uint iZ)
{
	CEquipYeti* pInstance = new CEquipYeti(pGraphicDev);
